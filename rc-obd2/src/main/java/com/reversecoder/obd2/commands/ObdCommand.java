...
import com.reversecoder.obd2.exceptions.*;
...
public abstract class ObdCommand {
    ...
    private static final String TAG = ObdCommand.class.getName();
    /**
    ...
    public void run(InputStream in, OutputStream out) throws IOException, InterruptedException {
        synchronized (ObdCommand.class) {//Only one command can write and read a data in one time.
            Log.d(TAG, "Executing command: " + this.getName());
            start = System.currentTimeMillis();
            Log.d(TAG, "Execution started at: " + ObdUtil.convertMilliSecondToTime(start, ""));
            sendCommand(out);
            readResult(in);
            end = System.currentTimeMillis();
            Log.d(TAG, "Execution ended at: " + ObdUtil.convertMilliSecondToTime(end, ""));
            Thread.sleep(250);
        }
    }
     ...
     * @throws java.lang.InterruptedException if any.
     */
    public void runRawCommand(InputStream in, OutputStream out) throws IOException, InterruptedException {
        synchronized (ObdCommand.class) {//Only one command can write and read a data in one time.
            Log.d(TAG, "Executing command: " + this.getName());
            start = System.currentTimeMillis();
            Log.d(TAG, "Execution started at: " + ObdUtil.convertMilliSecondToTime(start, ""));
...
//            readResult(in);
    ...
    protected void sendCommand(OutputStream out) throws IOException,
        ...
        Log.d(TAG, "sendCommand: " + (cmd + "\r").toString());
        out.flush();
    ...
    protected void fillBuffer() {
        ...
        Log.d(TAG, "fillBuffer: removed white space pattern" + rawData);
        rawData = removeAll(BUSINIT_PATTERN, rawData);
        Log.d(TAG, "fillBuffer: removed businit pattern" + rawData);

        if (!DIGITS_LETTERS_PATTERN.matcher(rawData).matches()) {
            Log.d(TAG, "fillBuffer: digit letters pattern does not matched");
        ...
        for (int i = 0; i < buffer.size(); i++) {
            Log.d(TAG, "fillBuffer: buffer " + i + " " + buffer);
        }
    }

    private void readBluetoothInputSteam(InputStream inputStream){
...
//                    Log.d("in.read(buffer);", new String(buffer));
//                }
//            } catch (IOException e) {
...
//                            });
     ...
     * @throws java.io.IOException if any.
     */
    protected void readRawCommand(InputStream in) throws IOException {
        byte b = 0;
...
//            }
        ...
        Log.d(TAG, "readRawData :" + res.toString());
        rawData=res.toString();
//        Log.d(TAG, "readRawData full: " + res.toString() + ">" + ObdUtil.getStringFromInputStream(in));
...
//
//        Log.d(TAG, "readRawData rawData after removing searching pattern: " + rawData);
//
//    /*
...
//        rawData = removeAll(WHITESPACE_PATTERN, rawData);//removes all [ \t\n\x0B\f\r]
//
//        Log.d(TAG, "readRawData rawData after removing white space pattern: " + rawData);
    }
    ...
    protected void readRawData(InputStream in) throws IOException {
        ...
        Log.d(TAG, "readRawData to >: " + res.toString());
//        Log.d(TAG, "readRawData full: " + res.toString() + ">" + ObdUtil.getStringFromInputStream(in));
    ...
    void checkForErrors() {
        for (Class<? extends ResponseException> errorClass : ERROR_CLASSES) {
            ...
            if (messageError.isError(rawData)) {
                Log.d(TAG, "checkForErrors: " + messageError.getMessage());
                throw messageError;
            }
        }
    }
