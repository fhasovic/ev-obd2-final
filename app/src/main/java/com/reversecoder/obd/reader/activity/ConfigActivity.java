...
public class ConfigActivity extends PreferenceActivity implements OnPreferenceChangeListener {
     ...
     */
        ...
        for (ObdCommand cmd : cmds) {
            ...
            if(cmd.getName().equalsIgnoreCase("Engine Load")
                    || cmd.getName().equalsIgnoreCase("Throttle Position")
                    || cmd.getName().equalsIgnoreCase("Engine RPM")
                    || cmd.getName().equalsIgnoreCase("Vehicle Speed")
                    || cmd.getName().equalsIgnoreCase("Trouble Codes")
                    || cmd.getName().equalsIgnoreCase("Control Module Power Supply")
                    || cmd.getName().equalsIgnoreCase("Distance traveled with MIL on")
                    || cmd.getName().equalsIgnoreCase("Diagnostic Trouble Codes")
                    || cmd.getName().equalsIgnoreCase("Timing Advance")
                    || cmd.getName().equalsIgnoreCase("Engine Coolant Temperature")
                    ){
